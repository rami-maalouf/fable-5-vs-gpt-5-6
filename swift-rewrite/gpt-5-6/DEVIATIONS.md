...
- Simulator verification covered session start, live timer updates, wake-up removal, setting disable and re-enable, force-quit relaunch, duplicate prevention, and a full simulator reboot. The compact island survived the reboot, the app restored the same active SQLite session after relaunch, and ending it cleared the island. Expanded and interactive Dynamic Island layouts still require the queued physical-device checkpoint because a simulator cannot prove hardware presentation behavior.
...
- Android was built and exercised on a Pixel 7 API 36 arm64 emulator. The first-run flow, notification permission, Home, Metrics, Logs, Settings, bedtime picker cancellation, session start, and wake-up all passed through the installed development build. A final fresh-emulator audit repeated all four tabs and the complete idle-active-idle session transition.
