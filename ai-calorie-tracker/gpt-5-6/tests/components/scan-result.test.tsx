...
describe("scan result UI", () => {
  it("uses an honest indeterminate overlay over the prepared photo", async () => {
    ...
    ).toHaveProp("source", [{ uri: "file:///prepared.jpg" }]);
    expect(screen.getByText("Analyzing your meal")).toBeOnTheScreen();
    expect(
      screen.getByText("Estimating calories and macros"),
    ).toBeOnTheScreen();
  ...
  it("reveals every estimate field without changing the photo URI or crop", async () => {
    ...
    const analyzingPhoto = screen.getByTestId("prepared-meal-photo", {
      includeHiddenElements: true,
    });
    ...
    const resultPhoto = screen.getByTestId("prepared-meal-photo", {
      includeHiddenElements: true,
    });
    expect(resultPhoto).toHaveProp("source", [{ uri: "file:///prepared.jpg" }]);
    expect(resultPhoto).toHaveProp("contentFit", "cover");
    expect(
      screen.getByText("AI ESTIMATE", { includeHiddenElements: true }),
    ).toBeOnTheScreen();
    ...
    expect(
      StyleSheet.flatten(accept.props.style).minHeight,
    ).toBeGreaterThanOrEqual(44);
    expect(
      StyleSheet.flatten(discard.props.style).minHeight,
    ).toBeGreaterThanOrEqual(44);
  });
});
